...
# Load the dataset
tumor_data <- read.csv("control3.csv", row.names = 1)  # Assuming gene names are row names
...
write.table(GetAssayData(seurat_cd3_positive, assay = "RNA", layer = "counts"), 
            "control3_CD3_positive.txt", sep = "\t", row.names = TRUE, col.names = NA, quote = FALSE)

# Print success message
cat("Filtered data saved to control3_CD4_positive.txt\n")

...
# Load the dataset
tumor_data <- read.csv("tumor2.csv", row.names = 1)  # Assuming gene names are row names
...
write.table(GetAssayData(seurat_cd3_positive, assay = "RNA", layer = "counts"), 
            "tumor2_CD3_positive.txt", sep = "\t", row.names = TRUE, col.names = NA, quote = FALSE)

# Print success message
cat("Filtered data saved to tumor2_CD3_positive.txt\n")

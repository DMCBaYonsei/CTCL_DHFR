...
#Determine the cutoff t-value corresponding to the top 51-100 genes
cut = abs(dat1$t)
cut = cut[order(-1*cut)]
cut_2 = cut[50]
cut_1 = cut[101]
...
win.metafile("F.meta_lymphoma_genes_2.wmf",width = 20, height = 5.5)
par(mar = c(11, 10, 5, 5))
...
plot(df, 
     ...
     cex.main = 1.8,
     main = "Top 51 - 100 CTCL_meta_genes", 
     xlab = "",
     ylab = "",
     border = T)
dev.off()
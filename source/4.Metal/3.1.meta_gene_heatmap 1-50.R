...
#Determine the cutoff t-value corresponding to the top 50 genes
cut = abs(dat1$t)
cut = cut[order(-1*cut)]
cut = cut[51]

# Filter genes with absolute t-value above the cutoff
dat1 = dat1[abs(dat1$t) > cut, ]
dat1 = dat1[order(-1*dat1$t), ]
...
win.metafile("F.meta_lymphoma_genes.wmf", width = 20, height = 5.5)
par(mar = c(10, 10, 5, 5))

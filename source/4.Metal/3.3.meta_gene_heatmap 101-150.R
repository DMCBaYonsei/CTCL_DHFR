...
#Determine the cutoff t-value corresponding to the top 101-150 genes
cut = abs(dat1$t)
cut = cut[order(-1*cut)]
cut_2 = cut[100]
cut_1 = cut[151]
...
win.metafile("F.meta_lymphoma_genes_3.wmf",width = 20, height = 5.5)
par(mar = c(10, 10, 5, 5))
...
plot(df, 
     ...
     cex.main = 1.8,
     main = "Top 101 - 150 CTCL_meta_genes", 
     xlab = "",
     ylab = "",
     border = T)
dev.off()

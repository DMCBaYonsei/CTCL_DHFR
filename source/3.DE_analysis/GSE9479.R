...
result %>%
  ...
  ggtitle("GSE9479 skin: Stage IIB vs early-stages") +
  theme_bw() +
...
#save plot
library(ggpubr)
ggsave("F.DEG_GSE9479.pdf", width = 8.5, height = 8)
...
diffexp <- data.frame(Symbol = result$Symbol,
                      ...
                      Sample_size = 51)
write.table(diffexp, "D:/R.data/DHFR_project/4.Metal/GSE9479.txt", row.names = FALSE, sep = "\t")
